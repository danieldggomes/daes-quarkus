...
| Name | Description | Default |
|------|------------|---------|
| `postgres.enabled` | Deploy embedded PostgreSQL | `true` |
| `postgres.image` | PostgreSQL image | `postgres:16` |
| `postgres.serviceName` | PostgreSQL service name | `db` |
| `postgres.auth.database` | Initial database name | `previdencia` |
| `postgres.auth.username` | PostgreSQL username | `postgres` |
